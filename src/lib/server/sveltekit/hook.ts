...
function parseSetCookie(cookie: string): ParsedCookie | null {
  ...
  for (const attribute of rawAttributes) {
    ...
    const key = (equalsIndex === -1 ? trimmed : trimmed.slice(0, equalsIndex))
      ...
      .toLowerCase();
    const rawValue =
      equalsIndex === -1 ? "" : trimmed.slice(equalsIndex + 1).trim();
...
function resolveBackend(
  ...
  )?.env?.[bindingName] as { fetch?: BackendFetch } | undefined;
      ...
      if (!locals.authDataFetched) {
        try {
          const authUrl = new URL(
            options.authPath ?? AUTH_GET_SESSION_PATH,
            event.request.url,
          );
          const authEvent = {
            ...event,
          ...
          console.error(
            "[app-kit/sveltekit] Failed to fetch auth data:",
            error,
          );
          locals.user = null;
          locals.session = null;
    ...
    const wrappedResolve: typeof resolve = (eventArg, resolveOptions) => {
      ...
      return resolve(eventArg, {
        ...
        transformPageChunk: async ({ html, done }) => {
          ...
          const rpc = locals[rpcStoreKey] as
