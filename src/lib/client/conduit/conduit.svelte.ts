...
class ConduitClient {
	...
	connect(url: string, options: ConduitOptions) {
		this.socket = new ReconnectingSocket(url, (raw) => this.dispatch(raw), options);
...
export function createConduit(
	...
	return {
		...
		off: <T = unknown>(key: string, fn: MessageHandler<T>) => client.off(key, fn),
		...
		call: <T = unknown>(key: string, data?: unknown) => client.call<T>(key, data),
		...
		init(url?: string) {
			...
			const resolvedUrl =
				...
				(typeof location !== "undefined"
					? location.origin.replace(/^https?/, (p) => (p === "https" ? "wss" : "ws")) +
